...
def log_exam_updated_added_event(sender, **kwargs):
	"""Writes information about newly added or updated exam into
	log file"""
	...
	if kwargs['created']:
		logger.info('Exam added: %s(%s) (ID: %d)', exam.subject,
			exam.exam_group, exam.id)
	else:
		logger.info('Exam updated: %s(%s) (ID: %d)', exam.subject,
			exam.exam_group, exam.id)
...
def log_exam_deleted_event(sender, **kwargs):
	...
	exam = kwargs['instance']
	logger.info('Exam deleted: %s(%s) (ID: %d)', exam.subject,
			exam.exam_group, exam.id)